...
test_feature_commits() {
    ...
    > "$GITHUB_OUTPUT"
    ./scripts/analyze-commits.sh "0.0.0" "true" > /tmp/test-output.txt 2>&1
...
test_fix_commits() {
    ...
    > "$GITHUB_OUTPUT"
    ./scripts/analyze-commits.sh "1.0.0" "false" > /tmp/test-output.txt 2>&1
...
test_commit_validation() {
    ...
    if ./scripts/validate-commits.sh "HEAD~2..HEAD" 2>/dev/null; then
...
test_release_notes() {
    ...
    > "$GITHUB_OUTPUT"
    ./scripts/analyze-commits.sh "1.0.0" "false" > /tmp/analysis-output.txt 2>&1
    ...
    ./scripts/generate-release-notes.sh "$NEXT_VERSION" "$VERSION_TYPE" "1.0.0" "$BREAKING_COUNT" "$FEAT_COUNT" "$FIX_COUNT" "$OTHER_COUNT" > /tmp/release-notes.txt
