...
use sage_core::{events::EventManager, CliOutput};
use sage_events::{EventId, undo::UndoOperation};
use sage_git::{
    repo::get_repo_root,
    branch::{switch, get_current},
};
use std::path::Path;
...
pub fn history() -> Result<()> {
    ...
    for (event, can_undo) in history.iter().rev() {
        ...
        let event_desc = match &event.data {
            sage_events::event::EventData::CommitCreated { message, branch, .. } => {
                let msg_preview = message.lines().next().unwrap_or("");
                format!("Commit on {}: {}", branch.yellow(), msg_preview)
            }
            sage_events::event::EventData::BranchCreated { name, from_branch, .. } => {
                format!("Create branch {} from {}", name.yellow(), from_branch.dimmed())
            }
            ...
            sage_events::event::EventData::Push { branch, commits, force, .. } => {
                let force_str = if *force { " (force)" } else { "" };
                format!("Push {} ({} commits){}", branch.yellow(), commits.len(), force_str.red())
            }
