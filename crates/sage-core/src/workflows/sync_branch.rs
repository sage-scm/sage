...
use sage_git::{
    branch::{get_current, get_default_branch, is_clean, merge, pull, push, switch, is_merge_in_progress, merge_abort, has_diverged, is_shared_branch},
    commit::commit,
    config::should_branch_rebase,
    rebase::{rebase, rebase_abort, rebase_continue, is_rebase_in_progress},
    repo::{fetch_remote, has_conflicts},
    stash::{stash_push, stash_pop},
    status::status,
};
...
fn should_use_rebase(branch: &str, graph: &SageGraph, explicit_rebase: Option<bool>) -> Result<bool> {
...
pub fn sync_branch(cli: &CliOutput, opts: &SyncBranchOpts) -> Result<()> {
    ...
    if should_sync_stack {
        ...
        if stack_size > 1 {
            cli.step_success(&format!("Stack detected: {}", branch_chain.join(" → ")), None);
        }
        ...
        for (i, branch) in branches_to_sync.iter().enumerate() {
            ...
            cli.step_start(&format!("[{}/{}] Syncing '{}'", i + 1, branches_to_sync.len(), branch));
            ...
            let use_rebase = should_use_rebase(branch, &graph, if opts.rebase { Some(true) } else { None })?;
    ...
    let use_rebase = should_use_rebase(&current_branch, &graph, if opts.rebase { Some(true) } else { None })?;
        ...
        Err(e) => {
            ...
            if e.to_string().contains("conflicts") || 
               ...
               is_merge_in_progress()? {
                ...
                cli.warning("Resolve conflicts and run 'sage sync --continue' to complete the sync");
